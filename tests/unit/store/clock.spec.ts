...
describe("store/clock", () => {
  ...
  it("simple", () => {
    ...
    expect(setting.onStopBeep.mock.calls.length).toBe(1);
    jest.advanceTimersByTime(239 * 1e3);
    // 1:01
    expect(clock.timeMs).toBe(61 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(0);
    jest.advanceTimersByTime(1 * 1e3);
    // 1:00
    expect(clock.timeMs).toBe(60 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(1);
    ...
    expect(setting.onBeepShort.mock.calls.length).toBe(2);
    jest.advanceTimersByTime(20 * 1e3);
    // 0:10
    expect(clock.timeMs).toBe(10 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(4);
    jest.advanceTimersByTime(4 * 1e3);
    // 0:06
    expect(clock.timeMs).toBe(6 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(8);
    expect(setting.onBeepUnlimited.mock.calls.length).toBe(0);
    jest.advanceTimersByTime(1 * 1e3);
    // 0:05
    expect(clock.timeMs).toBe(5 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(8);
    expect(setting.onBeepUnlimited.mock.calls.length).toBe(1);
    jest.advanceTimersByTime(4 * 1e3);
    // 0:01
    expect(clock.timeMs).toBe(1 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(8);
    expect(setting.onStopBeep.mock.calls.length).toBe(1);
    expect(setting.onTimeout.mock.calls.length).toBe(0);
    jest.advanceTimersByTime(1 * 1e3);
    // 0:00
    expect(clock.timeMs).toBe(0 * 1e3);
    expect(setting.onBeepShort.mock.calls.length).toBe(8);
    expect(setting.onStopBeep.mock.calls.length).toBe(2);
    expect(setting.onTimeout.mock.calls.length).toBe(1);
  });
  ...
  it("increment", () => {
    ...
    expect(setting.onStopBeep.mock.calls.length).toBe(2);
    clock.start();
    expect(setting.onStopBeep.mock.calls.length).toBe(3);
    ...
    expect(setting.onStopBeep.mock.calls.length).toBe(4);
  });
});
